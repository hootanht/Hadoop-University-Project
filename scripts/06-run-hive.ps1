...
# Removed --silent=true to enable full grid borders and headers for debugging
Invoke-LoggedCommand "docker exec hive-server beeline -u jdbc:hive2://localhost:10000 -f /query.hql" {
  docker exec hive-server beeline -u "jdbc:hive2://localhost:10000" -f /query.hql 2>&1 |
